...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    if (parsed.studentCode && parsed.studentCode !== student.studentCode) {
      ...
      if (ex) return NextResponse.json({ ok: false, error: "studentCode already exists" }, { status: 400 });
    }

    if (parsed.admissionNo && parsed.admissionNo !== student.admissionNo) {
      const ex = await Student.findOne({ schoolId: student.schoolId, admissionNo: parsed.admissionNo });
      if (ex) return NextResponse.json({ ok: false, error: "admissionNo already exists" }, { status: 400 });
    }
